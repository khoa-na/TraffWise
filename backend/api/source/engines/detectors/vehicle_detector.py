...
from ultralytics.nn.modules import head, transformer
...
# The bundled RT-DETR checkpoint was trained with renamed Ultralytics classes.
head.RTDETRDecoderV2 = head.RTDETRDecoder
transformer.DeformableTransformerDecoderLayerV2 = transformer.DeformableTransformerDecoderLayer
transformer.MSDeformAttnV2 = transformer.MSDeformAttn
...
class VehicleDetector:
    ...
    def load_model(self):
        ...
        if self.model_type == "yolo11":
            ...
            self.model = FRCNN(num_classes=5, pretrained=None)
    ...
    def switch_model(self, model_type: Literal["yolo11", "rtdetrv2", "faster_rcnn"]):
        with self._model_lock:
            ...
            try:
                ...
                self.model_type = previous_model_type
                raise
